...
```{r}
write.csv(clinic, "/home1/jdeiss/490_cluster/analysis_data/brca_clinical_data.csv", row.names=FALSE)
```

To read the csv back into your environment as a dataframe:

```{r}
clinic <- read.csv("/home1/jdeiss/490_cluster/analysis_data/brca_clinical_data.csv")
```
...
class(clinic$gender)
str(clinic$gender)
clinic[clinic$Tumor_Sample_Barcode=="TCGA-4H-AAAK","age_at_diagnosis"]
patient <- clinic[clinic$Tumor_Sample_Barcode == "TCGA-3C-AAAU", ]
patient$vital_status
patient$last_contact_days_to
patient$age_at_diagnosis + 3767/365
```
...
```{r Exercise 4.1}
plot(
  as.numeric(clinic$age_at_diagnosis),
  as.numeric(clinic$last_contact_days_to),
...
```{r Exercise 4.2}
boxplot(
  as.numeric(clinic$lymph_nodes_examined_count) ~ clinic$vital_status,
  main = "Lymph Nodes Examined by Vital Status",
  xlab = "Vital Status",
...
```{r}
under30_over60_mask <- as.numeric(clinic$age_at_diagnosis) < 30 | as.numeric(clinic$age_at_diagnosis) >60
```
...
cleaned_clinic$age_category <- ifelse(
  as.numeric(cleaned_clinic$age_at_diagnosis)<=35,"Young",
  ifelse(
    as.numeric(cleaned_clinic$age_at_diagnosis) < 50, "Middle", 
    "Old"
  )
)
```
...
death_num <- suppressWarnings(as.numeric(cleaned_clinic$death_days_to))
last_num  <- suppressWarnings(as.numeric(cleaned_clinic$last_contact_days_to))

cleaned_clinic$survival_time <- ifelse(
  cleaned_clinic$vital_status == "Dead",
  death_num,
  last_num
)
```
...
Middle age group as the red curve stays highest for most of the timeline, meaning a larger proportion of patients remain alive over time.
# 4. What age group appears to have the worst rate of survival? 
Old age group as the green curve drops lower than the others, indicating poorer survival overall. Even though, the Young group drops sharply later, that is likely due to fewer patients
# 5. What is the approx. 15-year survival rate for "Old" breast cancer patients?
28% survival rate (at around 5500 days)
...
p value = 0.2. Since the p-value is greater than 0.05, there is no strong statistical evidence that survival differences between age groups are real rather than due to random variation in this data set.
